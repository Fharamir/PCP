...
if __name__ == "__main__":
    ...
    supabase_client, main_user_id = get_authenticated_client(EMAIL_CLIENTE, PASSWORD_SICURA)
    if main_user_id:
        print("--- CONVERSATIONAL AGENT STARTED (type 'exit' to end) ---\n")
        while True:
            main_user_input = input("You: ").strip()
            if main_user_input.lower() in ["esci", "exit", "quit"]:
                print("--- SESSION TERMINATED ---")
                break
            # Skip processing for empty inputs
            if not main_user_input:
                continue
            process_input(supabase_client, main_user_id, main_user_input)
